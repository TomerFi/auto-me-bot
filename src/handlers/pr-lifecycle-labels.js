...
function getConfiguredLabels(labels) {
  return Object.fromEntries(
    Object.entries(labels).filter((kv) => KNOWN_LABELS.includes(kv[0]))
  );
}
...
async function getLifecycleLabel(context) {
  ...
  await context.octokit.pulls
    .listReviews(context.pullRequest())
    .then((response) => {
  ...
  let approvals = 0;
  Object.values(
    reviews.reduce((finalReviews, currentReview) => {
  ...
  let requiredApprovals =
    baseProtections?.required_pull_request_reviews
...
async function workThemLabels(context, config, report) {
  ...
  let removeLabels = KNOWN_LABELS.filter((l) => l !== lifecycleLabel)
  ...
  if (!prLabels.includes(addLabel)) {
    ...
    await context.octokit.issues
      .getLabel(context.repo({ name: addLabel }))
      ...
      .then((resp) => {
        if (resp.status !== 200) {
          ...
          report.output.summary =
            "This might be an internal time out, please try again";
        }
      })
      .catch((error) => {
        console.error(error);
