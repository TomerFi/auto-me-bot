  ...
  if (allCommits.length === 0) {
    ...
    await Promise.all(
      allCommits.map((commit) =>
        verifyCommitTrailer(commit.commit, config).catch(() =>
    ...
    if (numUnsignedCommits > 0) {
      ...
      report.output.text = unsignedCommits
...
function shouldSkipCommit(commit, config) {
  ...
  if (config && config.ignore) {
    if (config.ignore.emails) {
      if (
        config.ignore.emails.includes(commit.author.email) ||
    ...
    if (config.ignore.users) {
      if (
        config.ignore.users.includes(commit.author.name) ||
...
async function verifyCommitTrailer(commit, config) {
  ...
  commit.message.split(EOL).forEach((line) => {
    ...
    if (match !== null) {
      ...
      if (
        (signed.name === commit.author.name &&
          signed.email === commit.author.email) || // signed by author
        (signed.name === commit.committer.name &&
          signed.email === commit.committer.email) // signed by committer
      ) {
  ...
  return Promise.all(
    trailerMatches.map((match) =>
      emailVerifier.verify(match.email, (err, info) => {
        if (
          err ||
          info.code !== emailVerifier.verifyCodes.finishedVerification
        ) {
          return Promise.reject();
        }
      })
