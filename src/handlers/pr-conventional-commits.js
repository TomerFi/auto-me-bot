...
module.exports.match = function (context) {
  ...
  return event in context.payload
...
module.exports.run = async function (context, config, startedAt) {
  ...
  let checkRun = await context.octokit.checks.create(
    context.repo({
  ...
  let report = {
    ...
    output: {
      title: "Good Job!",
      summary:
        "Nothing to do here, no one told me you're a commit-message-master",
    },
  };
  // get the commits associated with the PR
  let commitObjs = [];
  await context.octokit.rest.pulls
    .listCommits(context.pullRequest()) // TODO: do we need "rest" here?
    .then((response) => {
  ...
  if (commitObjs.length === 0) {
    ...
    let lintStatuses = await commitObjs.map((commitObj) =>
      lintCommit(commitObj, opts)
    );
    // list warning and error statuses
    let errorStatuses = [];
    let warningStatuses = [];
    await Promise.all(lintStatuses).then((statuses) =>
    ...
    if (numError > 0) {
      ...
      if (numWarnings > 0) {
        ...
        title = `Found ${totalFound} non-conventional commit message${
          totalFound > 1 ? "s" : ""
        }`;
      } else {
        // found only errors - no warnings
        title = `Found ${numError} non-conventional commit message${
          numError > 1 ? "s" : ""
        }`;
      }
      ...
      report.output.text = errorStatuses
        .concat(warningStatuses)
        .map((lintSts) => parseLintStatus(lintSts))
        .join(EOL);
    } else if (numWarnings > 0) {
      // found only warning - no errors
      report.output.title = `Found ${numWarnings} non-conventional commit message${
        numWarnings > 1 ? "s" : ""
      }`;
      report.output.summary = "Take a look at these";
      report.output.text = warningStatuses
        .map((lintSts) => parseLintStatus(lintSts))
        .join(EOL);
    }
  }
